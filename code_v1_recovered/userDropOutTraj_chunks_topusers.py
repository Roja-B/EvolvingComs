...
for line in f:
	date = line.strip()
	cycleMap[date] = cycle
	filename = PATH+"/Results/1005"+date.replace("-","")+"/AllTopUsers"
	f1 = open(filename,"r")
	users = set()
	for line in f1:
...
TrajectoryNames = ["A","B","B1","B1prime","Bprime","E","Eprime"]
...
for TRAJECTORY in TrajectoryNames:
	...
	for line in f:
		...
		try: com = line.split("\t")[1]
			...
			print line
			break
		filename = "/Results/1005"+date.replace("-","")+"/topuser_community"+com
